...
function sendQuery(p) {
  ...
  var out;
  if(p.type==='calendar_request' && Number.isInteger(p.year)&&p.year>=2000&&p.year<=2100&&Number.isInteger(p.month)&&p.month>=1&&p.month<=12) { var c=monthDays(p.year,p.month), pack=function(x){return ((x[0]||0)|(x[1]||0)<<8|(x[2]||0)<<16|(x[3]||0)<<24)>>>0;}, mask=pack(c.mask); out={calendar_id:p.id,calendar_year:c.year,calendar_month:c.month,calendar_days:c.days,calendar_mask:mask,calendar_mask_a:pack(c.maskA),calendar_mask_b:pack(c.maskB)}; }
  if(p.type==='progress_request' && Number.isInteger(p.exercise)&&p.exercise>=0&&p.exercise<5&&Number.isInteger(p.page)&&p.page>=0) { var g=progress(p.exercise,p.page), chunks=Math.max(1,Math.ceil(g.points.length/5)); for(var ci=0;ci<chunks;ci++){var part=g.points.slice(ci*5,ci*5+5);out={progress_id:p.id,progress_exercise:g.exercise,progress_page:g.page,progress_total:g.total,progress_chunk_index:ci,progress_chunk_count:chunks,progress_point_count:part.length};part.forEach(function(pt,j){out['progress_t'+j]=pt.t;out['progress_w'+j]=pt.w;});try{Pebble.sendAppMessage(out);}catch(e){} } return; }
