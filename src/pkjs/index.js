...
function sendQuery(p) {
  ...
  if(p.type==='progress_request' && Number.isInteger(p.exercise)&&p.exercise>=0&&p.exercise<5&&Number.isInteger(p.page)&&p.page>=0) { console.log('PROGRESS_REQUEST id='+p.id+' exercise='+p.exercise+' page='+p.page); var g=progress(p.exercise,p.page), chunks=Math.max(1,Math.ceil(g.points.length/5)); for(var ci=0;ci<chunks;ci++){var part=g.points.slice(ci*5,ci*5+5);console.log('PROGRESS_RESPONSE id='+p.id+' exercise='+g.exercise+' page='+g.page+' total='+g.points.length+' chunk='+ci+'/'+chunks+' points='+part.length);out={progress_id:p.id,progress_exercise:g.exercise,progress_page:g.page,progress_total:g.total,progress_chunk_index:ci,progress_chunk_count:chunks,progress_point_count:part.length};part.forEach(function(pt,j){out['progress_t'+j]=pt.t;out['progress_w'+j]=pt.w;});try{Pebble.sendAppMessage(out,function(){console.log('PROGRESS_SEND_OK id='+p.id+' chunk='+ci);},function(e){console.log('PROGRESS_SEND_FAILED id='+p.id+' chunk='+ci+' code='+(e&&e.message||e||'unknown'));});}catch(e){console.log('PROGRESS_SEND_FAILED id='+p.id+' chunk='+ci+' code='+e);} } return; }
