...
static void retry(SyncMachine *m){m->state=SYNC_WAITING_RETRY;m->in_flight=false;m->timer_active=true;if(m->retry_index<4)m->retry_index++;}
...
void sync_machine_retry_elapsed(SyncMachine *m){if(m&&m->state==SYNC_WAITING_RETRY){m->state=SYNC_IDLE;m->timer_active=false;}}
