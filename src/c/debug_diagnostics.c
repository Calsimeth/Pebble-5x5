...
void debug_diag_state(const char *phase,const int32_t *weights,const uint8_t *plates,uint8_t count){if(!phase||!weights||!plates||count<7)return;APP_LOG(APP_LOG_LEVEL_INFO,"DBG_STATE_%s W=%ld,%ld,%ld,%ld,%ld P=%u,%u,%u,%u,%u,%u,%u",phase,(long)weights[0],(long)weights[1],(long)weights[2],(long)weights[3],(long)weights[4],plates[0],plates[1],plates[2],plates[3],plates[4],plates[5],plates[6]);}
