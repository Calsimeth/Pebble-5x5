...
void debug_diag_setup_change(uint8_t index,int32_t old_weight,int32_t new_weight){APP_LOG(APP_LOG_LEVEL_INFO,"SETUP_CHANGE i=%u old=%ld new=%ld",index,(long)(old_weight/4),(long)(new_weight/4));}
void debug_diag_save_begin(const char *reason,const int32_t *weights){if(!reason||!weights)return;APP_LOG(APP_LOG_LEVEL_INFO,"SAVE_BEGIN reason=%s W=%ld,%ld,%ld,%ld,%ld",reason,(long)(weights[0]/4),(long)(weights[1]/4),(long)(weights[2]/4),(long)(weights[3]/4),(long)(weights[4]/4));}
void debug_diag_save_commit(int result,uint32_t generation,uint8_t slot,const int32_t *weights){if(!weights)return;APP_LOG(APP_LOG_LEVEL_INFO,"SAVE_COMMIT result=%d generation=%lu slot=%u W=%ld,%ld,%ld,%ld,%ld",result,(unsigned long)generation,slot,(long)(weights[0]/4),(long)(weights[1]/4),(long)(weights[2]/4),(long)(weights[3]/4),(long)(weights[4]/4));}
void debug_diag_load_choice(const char *source,uint8_t version){APP_LOG(APP_LOG_LEVEL_INFO,"LOAD_CHOICE source=%s version=%u",source?source:"failure",version);}
void debug_diag_load_result(int result,uint32_t generation,uint8_t slot,const int32_t *weights){if(!weights)return;APP_LOG(APP_LOG_LEVEL_INFO,"LOAD_RESULT result=%d generation=%lu slot=%u W=%ld,%ld,%ld,%ld,%ld",result,(unsigned long)generation,slot,(long)(weights[0]/4),(long)(weights[1]/4),(long)(weights[2]/4),(long)(weights[3]/4),(long)(weights[4]/4));}
void debug_diag_ui_weights(const int32_t *weights){if(!weights)return;APP_LOG(APP_LOG_LEVEL_INFO,"UI_WEIGHTS W=%ld,%ld,%ld,%ld,%ld",(long)(weights[0]/4),(long)(weights[1]/4),(long)(weights[2]/4),(long)(weights[3]/4),(long)(weights[4]/4));}
void debug_diag_boot(void){APP_LOG(APP_LOG_LEVEL_INFO,"APP_BOOT");APP_LOG(APP_LOG_LEVEL_INFO,"DBG_KEYS %d %d",persist_get_size(debug_diag_key(0)),persist_get_size(debug_diag_key(1)));debug_diag_event(1,0,0);} void debug_diag_shutdown(void){APP_LOG(APP_LOG_LEVEL_INFO,"APP_DEINIT");debug_diag_event(2,0,0);} void debug_diag_clear(void){for(uint8_t i=0;i<=DBG_CAP;i++)persist_delete(debug_diag_key(i));APP_LOG(APP_LOG_LEVEL_INFO,"DBG_CLEAR");}
