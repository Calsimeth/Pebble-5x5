...
bool sync_adapter_start(SyncAdapter *a) {
  ...
  if (!a->timer || !a->timer(sync_machine_retry_delay(&a->machine),a->context)) { sync_machine_timeout(&a->machine); a->machine.timer_active=false; sync_adapter_schedule_retry(a); return false; }
  a->timer_registered=true; return true;
}
bool sync_adapter_transport(SyncAdapter *a, bool delivered) { if (!a) return false; if (!delivered) { enter_retry(a); return false; } sync_machine_transport(&a->machine,true); if (!a->timer || !a->timer(sync_machine_retry_delay(&a->machine),a->context)) { sync_machine_timeout(&a->machine); a->machine.timer_active=false; sync_adapter_schedule_retry(a); return false; } a->timer_registered=true; return true; }
