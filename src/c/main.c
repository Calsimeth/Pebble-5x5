...
static void history_progress_draw(Layer *layer, GContext *ctx) {
  ...
  if(s_screen==SCREEN_HISTORY && s_calendar_valid && s_calendar.mask) {
    ...
    if(min!=INT32_MAX){int axis_min=(min/25)*25,axis_max=((max+24)/25)*25;if(axis_max==axis_min)axis_max=axis_min+25;int plot_top=44,plot_height=72;for(int32_t tick=axis_min;tick<=axis_max;tick+=25){if(tick<min||tick>max)continue;graphics_context_set_stroke_color(ctx,(tick%50)==0?GColorLightGray:GColorDarkGray);int ty=plot_top+graph_coordinate(tick,axis_min,axis_max,plot_height);graphics_draw_line(ctx,GPoint(8,ty),GPoint(b.size.w-8,ty));}graphics_context_set_stroke_color(ctx,palette_accent());for(uint8_t i=1;i<s_progress_data.total_points;i++){int x0=(i-1)*b.size.w/(s_progress_data.total_points-1),x1=i*b.size.w/(s_progress_data.total_points-1);int y0=plot_top+graph_coordinate(s_progress_data.points[i-1].w,axis_min,axis_max,plot_height),y1=plot_top+graph_coordinate(s_progress_data.points[i].w,axis_min,axis_max,plot_height);graphics_draw_line(ctx,GPoint(x0,y0),GPoint(x1,y1));}for(uint8_t i=0;i<s_progress_data.total_points;i++){int x=s_progress_data.total_points==1?b.size.w/2:i*b.size.w/(s_progress_data.total_points-1);int y=plot_top+graph_coordinate(s_progress_data.points[i].w,axis_min,axis_max,plot_height);graphics_fill_rect(ctx,GRect(x-2,y-2,5,5),0);}char stats[64],range[32];struct tm *lo=localtime((time_t *)&s_progress_data.points[0].t),*hi=localtime((time_t *)&s_progress_data.points[s_progress_data.total_points-1].t);strftime(range,sizeof range,"%m/%d",lo);if(hi) {char tail[8];strftime(tail,sizeof tail,"-%m/%d",hi);strncat(range,tail,sizeof(range)-strlen(range)-1);}snprintf(stats,sizeof stats,"%s Min %ld Max %ld lb",range,(long)min,(long)max);graphics_draw_text(ctx,stats,fonts_get_system_font(FONT_KEY_GOTHIC_14),GRect(0,120,b.size.w,16),GTextOverflowModeFill,GTextAlignmentCenter,NULL);graphics_draw_text(ctx,"Page 1/1",fonts_get_system_font(FONT_KEY_GOTHIC_14),GRect(0,136,b.size.w,16),GTextOverflowModeFill,GTextAlignmentCenter,NULL);}
