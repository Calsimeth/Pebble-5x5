...
static void history_progress_draw(Layer *layer, GContext *ctx) {
  ...
  if(s_screen==SCREEN_HISTORY && s_calendar_valid) {
    ...
    int left=PBL_IF_ROUND_ELSE(34,0), bottom=PBL_IF_ROUND_ELSE(26,0), grid_width=b.size.w-left*2, cw=grid_width/7, top=46, ch=(b.size.h-top-bottom)/6;
    static const char *weekdays[] = {"S","M","T","W","T","F","S"};
    for(int x=0;x<7;x++) graphics_draw_text(ctx,weekdays[x],fonts_get_system_font(FONT_KEY_GOTHIC_14),GRect(left+x*cw,28,cw,14),GTextOverflowModeFill,GTextAlignmentCenter,NULL);
