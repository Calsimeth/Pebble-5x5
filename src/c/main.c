...
static void history_progress_draw(Layer *layer, GContext *ctx) {
  ...
  if(s_screen==SCREEN_HISTORY && s_calendar_valid) {
    ...
    int left=PBL_IF_ROUND_ELSE(40,0), bottom=PBL_IF_ROUND_ELSE(30,0), grid_width=b.size.w-left*2, cw=grid_width/7, top=48, ch=(b.size.h-top-bottom)/6;
