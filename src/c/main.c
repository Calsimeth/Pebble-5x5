...
static void history_progress_draw(Layer *layer, GContext *ctx) {
  ...
  if(s_screen==SCREEN_HISTORY && s_calendar_valid) {
    ...
    for(int d=1;d<=s_calendar.days;d++){int n=first+d-1,x=n%7,y=n/7;bool marked=(s_calendar.mask&(1u<<(d-1)))!=0;char v[4];snprintf(v,sizeof v,"%d",d);if(marked){graphics_context_set_fill_color(ctx,palette_accent());graphics_fill_circle(ctx,GPoint(x*cw+cw/2,top+y*ch+ch/2),cw/2-1);graphics_context_set_text_color(ctx,PBL_IF_COLOR_ELSE(GColorWhite,GColorBlack));}else graphics_context_set_text_color(ctx,GColorWhite);graphics_draw_text(ctx,v,fonts_get_system_font(FONT_KEY_GOTHIC_14),GRect(x*cw+2,top+y*ch+1,cw-3,ch-1),GTextOverflowModeFill,GTextAlignmentCenter,NULL);}
