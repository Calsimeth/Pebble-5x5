...
static void history_progress_draw(Layer *layer, GContext *ctx) {
  ...
  if(s_screen==SCREEN_HISTORY && s_calendar_valid) {
    ...
    for(int d=1;d<=s_calendar.days;d++){int n=first+d-1,x=n%7,y=n/7;uint32_t bit=1u<<(d-1);bool marked=(s_calendar.mask&bit)!=0;char v[4];snprintf(v,sizeof v,"%d",d);if(marked){bool a=(s_calendar.mask_a&bit)!=0,bm=(s_calendar.mask_b&bit)!=0;graphics_context_set_fill_color(ctx,palette_accent());if(a&&bm)graphics_fill_rect(ctx,GRect(left+x*cw+1,top+y*ch+1,cw-2,ch-2),0,GCornerNone);else if(a)graphics_fill_rect(ctx,GRect(left+x*cw+1,top+y*ch+1,(cw-2)/2,ch-2),0,GCornerNone);else if(bm)graphics_fill_rect(ctx,GRect(left+x*cw+1+(cw-2)/2,top+y*ch+1,cw-2-(cw-2)/2,ch-2),0,GCornerNone);graphics_context_set_text_color(ctx,PBL_IF_COLOR_ELSE(GColorWhite,GColorBlack));}else graphics_context_set_text_color(ctx,GColorWhite);graphics_draw_text(ctx,v,fonts_get_system_font(FONT_KEY_GOTHIC_14),GRect(left+x*cw+1,top+y*ch+1,cw-2,ch-2),GTextOverflowModeFill,GTextAlignmentCenter,NULL);}
