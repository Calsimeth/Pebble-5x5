...
static void history_progress_draw(Layer *layer, GContext *ctx) {
  ...
  if(s_screen==SCREEN_HISTORY && s_calendar_valid && s_calendar.mask) {
    ...
    int margin=PBL_IF_ROUND_ELSE(48,0), bottom=PBL_IF_ROUND_ELSE(36,0), top=50, ch=(b.size.h-top-bottom)/6, cw=(b.size.w-margin*2)/7, grid_width=cw*7, left=(b.size.w-grid_width)/2;
    ...
    for(int d=1;d<=s_calendar.days;d++){int n=first+d-1,x=n%7,y=n/7;uint32_t bit=1u<<(d-1);bool marked=(s_calendar.mask&bit)!=0;char v[4];snprintf(v,sizeof v,"%d",d);bool a=(s_calendar.mask_a&bit)!=0,bm=(s_calendar.mask_b&bit)!=0;if(marked){graphics_context_set_fill_color(ctx,palette_accent());graphics_fill_rect(ctx,GRect(left+x*cw,top+y*ch,cw,ch),0,GCornerNone);graphics_context_set_text_color(ctx,PBL_IF_COLOR_ELSE(GColorWhite,GColorBlack));}else graphics_context_set_text_color(ctx,GColorWhite);graphics_draw_text(ctx,v,fonts_get_system_font(PBL_IF_COLOR_ELSE(FONT_KEY_GOTHIC_14,FONT_KEY_GOTHIC_09)),GRect(left+x*cw+1,top+y*ch+1,cw-2,ch-8),GTextOverflowModeFill,GTextAlignmentCenter,NULL);if(marked){char which[3];snprintf(which,sizeof which,"%s",a&&bm?"AB":(bm?"B":"A"));graphics_draw_text(ctx,which,fonts_get_system_font(FONT_KEY_GOTHIC_09),GRect(left+x*cw+1,top+y*ch+ch-10,cw-2,5),GTextOverflowModeFill,GTextAlignmentCenter,NULL);}}
    ...
    if(min!=INT32_MAX){int32_t axis_min,axis_max;bool includes_zero=false;for(uint8_t i=0;i<s_progress_data.total_points;i++)if(s_progress_data.points[i].w==0)includes_zero=true;graph_axis_bounds(min,max,includes_zero,&axis_min,&axis_max);int plot_top=44,plot_height=68,plot_inset=2,plot_left=5,plot_width=b.size.w-10;for(int32_t tick=axis_min;tick<=axis_max;tick+=25){graphics_context_set_stroke_color(ctx,(tick%50)==0?GColorLightGray:GColorDarkGray);int ty=plot_top+plot_inset+graph_coordinate(tick,axis_min,axis_max,plot_height-2*plot_inset);graphics_draw_line(ctx,GPoint(plot_left,ty),GPoint(plot_left+plot_width-1,ty));}graphics_context_set_stroke_color(ctx,palette_accent());for(uint8_t i=1;i<s_progress_data.total_points;i++){int x0=plot_left+graph_x_coordinate(i-1,s_progress_data.total_points,plot_width),x1=plot_left+graph_x_coordinate(i,s_progress_data.total_points,plot_width);int y0=plot_top+plot_inset+graph_coordinate(s_progress_data.points[i-1].w,axis_min,axis_max,plot_height-2*plot_inset),y1=plot_top+plot_inset+graph_coordinate(s_progress_data.points[i].w,axis_min,axis_max,plot_height-2*plot_inset);graphics_draw_line(ctx,GPoint(x0,y0),GPoint(x1,y1));}for(uint8_t i=0;i<s_progress_data.total_points;i++){int x=plot_left+graph_x_coordinate(i,s_progress_data.total_points,plot_width);int y=plot_top+plot_inset+graph_coordinate(s_progress_data.points[i].w,axis_min,axis_max,plot_height-2*plot_inset);graphics_fill_rect(ctx,GRect(x-2,y-2,5,5),0,GCornerNone);}char stats[40],range[32];struct tm *lo=localtime((time_t *)&s_progress_data.points[0].t),*hi=localtime((time_t *)&s_progress_data.points[s_progress_data.total_points-1].t);strftime(range,sizeof range,"%m/%d",lo);if(hi) {char tail[8];strftime(tail,sizeof tail,"-%m/%d",hi);strncat(range,tail,sizeof(range)-strlen(range)-1);}snprintf(stats,sizeof stats,"Min %ld Max %ld lb",(long)min,(long)max);graphics_draw_text(ctx,range,fonts_get_system_font(FONT_KEY_GOTHIC_14),GRect(0,116,b.size.w,16),GTextOverflowModeFill,GTextAlignmentCenter,NULL);graphics_draw_text(ctx,stats,fonts_get_system_font(FONT_KEY_GOTHIC_14),GRect(0,132,b.size.w,16),GTextOverflowModeFill,GTextAlignmentCenter,NULL);graphics_draw_text(ctx,"Page 1/1",fonts_get_system_font(FONT_KEY_GOTHIC_14),GRect(0,148,b.size.w,16),GTextOverflowModeFill,GTextAlignmentCenter,NULL);}
...
static void rest_tick(struct tm *tick_time, TimeUnits units_changed) {
  ...
  RestState rest = { .active=s_state.rest_active, .halfway_alerted=s_state.halfway_alerted, .completion_alerted=s_state.completion_alerted, .five_minute_alerted=rest_five_minute_alerted_from_end(s_state.rest_end), .start=s_state.rest_start, .elapsed=s_state.rest_elapsed };
  int alerts = rest_alerts_due(&rest, (int32_t)now);
  s_state.rest_elapsed = rest.elapsed; s_state.halfway_alerted = rest.halfway_alerted; s_state.completion_alerted = rest.completion_alerted; s_state.rest_end = rest_end_with_five_minute_alert(rest.five_minute_alerted);
  if (alerts & 1) {
    vibes_short_pulse();
    save_state();
  }
...
static void query_received(DictionaryIterator *i) {
  ...
  if (p) { Tuple *ex=dict_find(i,MESSAGE_KEY_progress_exercise),*pg=dict_find(i,MESSAGE_KEY_progress_page),*tt=dict_find(i,MESSAGE_KEY_progress_total),*ix=dict_find(i,MESSAGE_KEY_progress_chunk_index),*cc=dict_find(i,MESSAGE_KEY_progress_chunk_count),*pc=dict_find(i,MESSAGE_KEY_progress_point_count); ProgressPoint pts[5];uint8_t n=pc?pc->value->uint8:0;for(uint8_t z=0;z<n&&z<5;z++){Tuple *t=dict_find(i,MESSAGE_KEY_progress_t0+z),*w=dict_find(i,MESSAGE_KEY_progress_w0+z);if(!t||!w){n=6;break;}pts[z]=(ProgressPoint){t->value->int32,w->value->uint16};} APP_LOG(APP_LOG_LEVEL_INFO,"PROGRESS_RESPONSE id=%u exercise=%u page=%u chunk=%u/%u points=%u",p->value->uint16,ex?ex->value->uint8:255,pg?pg->value->uint8:255,ix?ix->value->uint8:255,cc?cc->value->uint8:255,n); bool accepted=ex&&pg&&tt&&ix&&cc&&n<=5&&progress_chunk_add(&s_progress_data,p->value->uint16,ex->value->uint8,pg->value->uint8,tt->value->uint8,ix->value->uint8,cc->value->uint8,n,pts); APP_LOG(APP_LOG_LEVEL_INFO,"PROGRESS_%s id=%u",accepted?"ACCEPTED":"REJECTED",p->value->uint16); if(accepted&&progress_assembly_complete(&s_progress_data)){APP_LOG(APP_LOG_LEVEL_INFO,"PROGRESS_RENDER %s points=%u",s_progress_data.total_points?"graph":"No Progress",s_progress_data.total_points);s_query_connected=true;query_controller_response(&s_query_controller,true,true);if(s_query_timer){app_timer_cancel(s_query_timer);s_query_timer=NULL;}send_oldest();update_display();} }
}
