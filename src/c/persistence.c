...
uint32_t persistence_last_generation(void){return last_generation;}
uint8_t persistence_last_slot(void){return last_slot;}
...
PersistenceResult persistence_save(const PersistenceAdapter*a,size_t cs,const void*c,size_t ss,const void*s,void*x){
 ...
 PersistenceMetadata m={0};uint8_t slot=0;
 if (a->exists && a->exists(META,x)) {
  if (!a->read || a->read(META,&m,sizeof m,x)!=(int)sizeof m) return PERSIST_IO;
  if (!m.valid || m.slot>1) return PERSIST_INVALID;
  slot=m.slot^1u;
 }
 if(a->write(key(CORE0,slot),c,cs,x)!=(int)cs||a->write(key(SYNC0,slot),s,ss,x)!=(int)ss)return PERSIST_IO;
 PersistenceMetadata n={(m.valid?m.generation:0u)+1u,slot,1};if(a->write(META,&n,sizeof n,x)!=(int)sizeof n)return PERSIST_IO;last_generation=n.generation;last_slot=n.slot;return PERSIST_OK;
}
PersistenceResult persistence_load(const PersistenceAdapter*a,size_t cs,void*c,size_t ss,void*s,void*x,PersistenceMetadata*out){
 ...
 last_generation=m.generation;last_slot=m.slot;if(out)*out=m;
 return PERSIST_OK;
}
